...
PMSF, LG+C20+F+R, and ASTRAL strategies. This section walks through the full 
pipeline and demonstrates how to read the output.
...
The Culicomorpha trees include three outgroup taxa from outside the infraorder:
Phlebotomus chinensis and Clogmia albipunctata (Psychodidae) and Coboldia fuscipes (Scatopsidae). 
We will root on all three, then drop them. The order matters: you must root 
while the outgroup tips are still present, because `ape::root()` needs to find 
them in the tree.
...
The Culicomorpha trees already carry species names as tip labels, so no 
translation step is needed. When your trees use specimen codes instead (as in 
the beetles example below), use `translate_tips()` to map them to species names 
before building the matrix.
##Choose a backbone

Now we will pick one analysis as the backbone. The remaining trees become 
comparison trees. 
```{r backbone and comparison selection}
backbone <- trees[["Matrix1-kpi_PMSF(H1.guide)"]]
others   <- trees[names(trees) != "Matrix1-kpi_PMSF(H1.guide)"]
```
##Check taxa
Before comparing trees, verify that every comparison tree carries the same taxa 
as the backbone. check_taxa() flags any mismatches. If taxa differ, use 
prune_to_shared() to reduce all trees to their common set.
```{r checktaxa-culicomorpha}
check_taxa(backbone, others)
```
##Build the node presence matrix
```{r npm-culicomorpha}
...
```{r presence-mode-culicomorpha}
plot_phylorug(backbone, npm)
```
With 9 comparison trees, phylorug automatically arranges the rug grid (3 columns
by 3 rows). Black dots mark nodes where all 9 analyses agree. Mixed grids mark 
the contested nodes. Look at the deeper backbone nodes, the monophyly of the 
infraorder Culicomorpha and its major families should be unanimously recovered 
(black dots), while the interrelationships among families are where the 
disagreements appear.

##Support mode
Now we will define the support metric for each comparison tree. The two ASTRAL 
trees carry local posterior probabilities (`"lpp"`), while all IQ-TREE analyses 
carry `SH-aLRT values (`"sh_alrt"`):
```{r define-support-type-culicomorpha}
support_type <- c(
  "Matrix1-kpi_ASTRAL"            = "lpp",
  "Matrix1-smart_ASTRAL"          = "lpp",
  "Matrix1-kpi_partitioning"      = "sh_alrt",

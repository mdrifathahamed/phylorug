...
comparison tree and reports any mismatches, missing taxa, extra taxa, or
spelling differences. Running it before `node_presence_matrix()` is good
practice, especially the first time you work with a new dataset. That
said, `node_presence_matrix()` will stop with a clear error if a
...
`node_presence_matrix()` is the core function of the pipeline. It walks
every internal node of the backbone, checks whether each comparison tree
recovered that clade, and records its support value.
...
`100/98` (SH-aLRT/UFBoot2 from IQ-TREE, for example), `support_col` picks which 
one to use: `support_col = 1` for the first, `support_col = 2` for the second, 
or `support_col = c(1, 2)` to pull both at once and makes two 
**support matrices**.

Two more arguments belong here. `support_type` is a named character
vector telling `node_presence_matrix()` which support metric each
comparison tree uses (`"ufboot"`, `"lpp"`, `"sh_alrt"`, `"posterior"`).
...
what `plot_phylorug()` uses to draw the rug: presence determines whether
a cell is filled or empty; support determines its shade.
...
recovered but no support value could be parsed.The thresholds shown below are 
the package defaults. Different fields and journals apply different cutoffs, so 
we recommend defining your own via the `thresholds` argument (see the 
...
- **Red**: clade recovered but no support value could be parsed from
  the node label. This can happen when the tree file carries no node
  labels, the label is not numeric, or the original analysis did not
  report support at that node.
...
and *Coboldia fuscipes (Scatopsidae)*. Root on all three,
then drop them. The order matters, you must root while the outgroup tips
are still present, because `ape::root()` needs to find them in the tree:
...
display only the first support metric instead of the full compound
label (e.g. showing `100` rather than `100/100`). `cell_scale = 0.3`
shrinks the rug cells slightly for a denser tree. All-white rugs are
hidden by default, so only contested and stable nodes remain visible.

```{r plot-culico-support,fig.width = 8, fig.height = 10, fig.dpi=300}
plot_phylorug(backbone, npm,
              mode               = "support",
              show_support       = TRUE,
              show_support_idx   = 1,
...
root and drop outgroups before translating, because `translate_tips()`
...
```{r plot-support-beetles-70p, fig.height = 12, fig.width = 10, fig.dpi= 300}
# For publication output, add:
...
matrix, `plot_phylorug()` falls back to universal thresholds. Values
greater than 1 are divided by 100 to bring them onto a 0–1 scale, and
all trees are binned against the same cutoffs regardless of metric. This
is useful for quick exploration but less precise than metric-specific
binning:
...
By default, phylorug chooses a roughly square grid. Override with `n_rows` and 
`n_cols`:
...
- Fu, Y., Du, S., Fang, X., Xu, Z. & Wang, X. (2025). Phylogenomic
  ...
  (Diptera) revealed by whole-genome sequencing. *Insect Systematics
  and Diversity*, 9(6), ixaf056.
...
- Sanders, J.G. (2010). Program note: Cladescan, a program for
  ...
  114–116. https://doi.org/10.1111/j.1096-0031.2009.00280.x

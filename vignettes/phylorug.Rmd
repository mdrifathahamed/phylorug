...
same set of taxa — different locus sets, alignment strategies,
substitution models, and inference methods each yield a tree that may
agree or disagree with the others. The question every systematist faces
is: **which clades are robust across analyses, and which are sensitive
to analytical choices?**

Answering that question by flipping between tree files is tedious and
error-prone. phylorug solves it by drawing a small grid of coloured
cells — a **rug** — at every internal node of a single reference tree
(the backbone). Each cell in the grid represents one analysis, and its
colour tells you whether that analysis recovered the same clade, and how
strongly it supported it.
...
2015). phylorug brings this idea into R as a native package, with
automatic handling of heterogeneous support formats (UFBoot2, SH-aLRT,
ASTRAL local posterior probability, Bayesian posterior probability)
across independently run pipelines.

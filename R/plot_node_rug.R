...
#'
#' For each internal node in the backbone tree, draws a compact grid of
#' rectangles representing the corresponding clade across comparison trees.
#' Each rectangle indicates whether the clade is recovered and, in support
#' mode, the strength of support for that clade. The collection of these
#' node-level grids constitutes the rug.
#'
#' The visual mode is resolved automatically from what was supplied:
#'
#' * Presence mode (`support` is `NULL`): black = clade recovered,
#'   white = absent. For pooled analyses (multiple equally parsimonious
#'   trees stored as a single entry), cells are greyscale in proportion
#'   to the fraction of trees recovering the clade.
#' * Support mode (both `support` and `support_type` given): recovered
#'   clades are shaded by binned support (greyscale from black for very
#'   high to white-with-dots for low); absent clades are white;
#'   not-evaluable clades are crossed.
#'
#' Users do not call this directly; [plot_phylorug()] calls it after
#' drawing the tree and computing the cell geometry.
#'
#' @param presence Presence matrix from [node_presence_matrix()]: one row
#'   per internal backbone node (node numbers as rownames), one column per
#'   comparison tree. Cells are `1`, `0`, or a pool proportion.
#' @param support Support matrix of the same shape, or `NULL`. When `NULL`,
#'   the rug is drawn in presence mode.
#' @param support_type Named character vector mapping each comparison tree
#'   to its support measure (`"ufboot"`, `"sh_alrt"`, `"lpp"`,
#'   `"posterior"`), or `NULL`. Required for binned shading.
#' @param thresholds Optional list overriding the built-in bin thresholds,
#'   keyed by support type. `NULL` uses the literature defaults.
#' @param cell_h,cell_w Numeric. Height and width of one cell, in the
#'   tree's plotting coordinates (not inches).
#' @param n_cols Integer. Number of columns in each node's grid.
#' @param x_offset,y_offset Numeric. Shift the whole grid away from the
#'   node, as a fraction of the tree's width and height. Default `0`.
#' @param rug_position One of `"outside"` (default) or `"inside"`.
#'   `"outside"` places the grid to the right of the node; `"inside"`
#'   tucks it into the crook above-left.
#' @param last_pp The stored [ape::plot.phylo()] coordinates. If `NULL`,
#'   fetched from the active device via `ape::.PlotPhyloEnv`.
#'
#' @returns Invisibly `NULL`; called for its drawing side effect.
#'
#' @seealso [plot_phylorug()] which calls this function, and
#'   [node_presence_matrix()] which builds the input matrices.
#'
#' @keywords internal

...
#
# The drawing itself needs an active graphics device, so most tests target the
# pure decision logic: resolve_tier(), resolve_cell(), bin_support(). These
# carry the four-state grammar and the no-normalization rule, so they are where
# correctness matters.
...
test_that("NA presence is hatched in every tier", {
  for (tier in 1:3) {
    cell <- resolve_cell(NA_real_, NA_real_, NA_character_, NULL, tier)
    expect_true(cell$hatch)
  }
})
...
test_that("absent clade is white in tiers 1 and 2", {
  ...
  expect_false(c1$hatch)
})

test_that("absent clade is yellow in tier 3", {
  c3 <- resolve_cell(0, NA_real_, "ufboot", NULL, 3L)
  expect_equal(c3$fill, "#E8C547")
  expect_false(c3$hatch)
})
...
test_that("tier 3 shades a present clade by its binned support", {
  # UFBoot 99 -> very high -> bin 4 -> black
  c_high <- resolve_cell(1, 99, "ufboot", NULL, 3L)
  expect_equal(c_high$fill, bin_fill(4L))

  # UFBoot 40 -> low -> bin 1 -> red
  c_low <- resolve_cell(1, 40, "ufboot", NULL, 3L)
  expect_equal(c_low$fill, bin_fill(1L))
})
...
test_that("UFBoot binning follows the default thresholds", {
  expect_equal(bin_support(96,  "ufboot", NULL), 4L)   # >= 95 very high
  expect_equal(bin_support(95,  "ufboot", NULL), 4L)   # boundary
  expect_equal(bin_support(70,  "ufboot", NULL), 2L)   # 50-94 moderate
  expect_equal(bin_support(40,  "ufboot", NULL), 1L)   # < 50 low
})

test_that("SH-aLRT binning follows the default thresholds", {
  expect_equal(bin_support(99, "sh_alrt", NULL), 4L)   # >= 98 very high
  expect_equal(bin_support(85, "sh_alrt", NULL), 3L)   # 80-97.9 high
  expect_equal(bin_support(60, "sh_alrt", NULL), 2L)   # 50-79.9 moderate
  expect_equal(bin_support(30, "sh_alrt", NULL), 1L)   # < 50 low
})

test_that("ASTRAL LPP binning follows the default thresholds", {
  expect_equal(bin_support(0.99, "lpp", NULL), 4L)     # >= 0.99 very high
  expect_equal(bin_support(0.96, "lpp", NULL), 3L)     # 0.95-0.98 high
  expect_equal(bin_support(0.70, "lpp", NULL), 2L)     # 0.5-0.94 moderate
  expect_equal(bin_support(0.30, "lpp", NULL), 1L)     # < 0.5 low
})

test_that("the same numeric value bins differently by support type", {
  # This is the whole point: 96 is very-high UFBoot but only high SH-aLRT is
  # a different story; here 85 is high SH-aLRT but moderate UFBoot.
...
test_that("user thresholds override the defaults", {
  ...
  expect_equal(bin_support(95, "ufboot", custom), 3L)  # now only high, not vh
  expect_equal(bin_support(80, "ufboot", custom), 2L)  # moderate
  expect_equal(bin_support(60, "ufboot", custom), 1L)  # low
})

test_that("unknown support type falls back to a percentage scale", {
  # Should not error; uses the generic 95/80/50 fallback.
...
test_that("bin_fill maps each bin to a distinct fill", {
  fills <- vapply(1:4, bin_fill, character(1))
  expect_length(unique(fills), 4L)
})

test_that("bin_fill returns black for NA (present but unquantified)", {
  expect_equal(bin_fill(NA_integer_), "black")
})
